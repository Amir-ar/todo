<template>
  <div class="task-box">
    ...
    <div class="task" v-for="task in myJosn" :key="task.id">
      <button class="check" @click="isDone(task.id)">
        <i class="fa fa-check" :class="{ done: task.done }" aria-hidden="true"></i>
      </button>
      ...
      <button class="important" @click="important(task.id)">
        <i class="fa fa-star-o" :class="{isImp:task.isImportant}" aria-hidden="true"></i>
        <i class="fa fa-star" :class="{isImpor:task.isImportant , notimpor:!task.isImportant}" aria-hidden="true"></i>
      </button>
    </div>
...
export default {
  ...
  methods: {
    isDone(userid) {
      for (var i in this.myJosn) {
        if (this.myJosn[i].id == userid) {
          this.myJosn[i].done = !this.myJosn[i].done;
          break;
        }
      }
    },
    addNew() {
      ...
      this.taskTitle = "";
      console.log(this.myJosn);
      const data = JSON.stringify(this.myJosn)
      window.localStorage.setItem('myJosn', data);
    },
    removeTask(taskId){
      for (var i in this.myJosn) {
        if(this.myJosn[i].id == taskId){
                this.myJosn.splice(i,1)
                break;
            } 
      }
    },
    important(taskId){
        for(var i in this.myJosn){
            if(this.myJosn[i].id == taskId){
                this.myJosn[i].isImportant = !this.myJosn[i].isImportant;
                break;
            }  
        }
    },
    editTask(taskId){
      for(var i in this.myJosn){
            if(this.myJosn[i].id == taskId){
                this.taskTitle = this.myJosn[i].title;
                this.taskPersonal = this.myJosn[i].personal;
                this.id=this.myJosn[i].id;
                this.done=this.myJosn[i].isDone;
                this.isImportant=this.myJosn[i].isImportant;
                break;
            }  
        }
        for (var i in this.myJosn) {
...
/* fa-trash-o
fa-star-o */
button.delete,button.important,button.Edit{
    background-color: transparent;
    color: #ccc;
    border: 0;
    font-size: 1.4rem;
